...
void hsvToRgb(const HsvColor& hsvColor, RgbColor& rgbColor)
{
  double r, g, b;
  double h = hsvColor.h;
  double s = hsvColor.s;
  ...
  rgbColor.r = static_cast<int>(r * 255.0);
  rgbColor.g = static_cast<int>(g * 255.0);
  rgbColor.b = static_cast<int>(b * 255.0);
  return;
}

void rgbToHsv(const RgbColor& rgbColor, HsvColor& hsvColor)
{
  double rd = static_cast<double>(rgbColor.r / 255.0);
  double gd = static_cast<double>(rgbColor.g / 255.0);
  double bd = static_cast<double>(rgbColor.b / 255.0);
  double maxValue = std::max(std::max(rd, gd), bd);
  double minValue = std::min(std::min(rd, gd), bd);
  double h;
  double s;
  double v = maxValue;
  double deltaValue = maxValue - minValue;
  s = maxValue == 0 ? 0 : deltaValue / maxValue;
  if(maxValue == minValue) {
    h = 0;
  }
  else {
    if (maxValue == rd) {
        h = (gd - bd) / deltaValue + (gd < bd ? 6.0 : 0.0);
    } else if (maxValue == gd) {
        h = (bd - rd) / deltaValue + 2.0;
    } else if (maxValue == bd) {
        h = (rd - gd) / deltaValue + 4.0;
    }
    h = h / 6.0;
  }
  hsvColor.h = h;
...
void setNeoPixelColors(int colors[][4], bool isSetColorsBrightness, int startId, int endId) {
  for (int i = startId; i < endId; i++) {
    if (isSetColorsBrightness)
    {
      HsvColor hsvColor;
      RgbColor rgbColor;
      rgbColor.r = colors[i][0];
      rgbColor.g = colors[i][1];
      rgbColor.b = colors[i][2];
      rgbToHsv(rgbColor, hsvColor);
      hsvColor.v = static_cast<double>(colors[i][3] / 255.0);
...
{
  if(isDebugEnabled && !isTaskRunning)
  {
    Serial.println("LED RGB Color: " + String(colors[0]) + " " + String(colors[1]) + " " + String(colors[2]));
    Serial.println("LED Brightness: " + String(colors[3]));
  }
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    rgbColor.b = colors[2];
    rgbToHsv(rgbColor, hsvColor);
    if(isDebugEnabled && !isTaskRunning && i == 0)
    {
      Serial.println("LED HSV Color: " + String(hsvColor.h) + " " + String(hsvColor.s) + " " + String(hsvColor.v));
      Serial.println("New HSV.V: " + String(static_cast<double>(colors[3] / 255.0)));
    }
    hsvColor.v = static_cast<double>(colors[3] / 255.0);
    hsvToRgb(hsvColor, rgbColor);
    if(isDebugEnabled && !isTaskRunning && i == 0)
    {
      Serial.println("New LED RGB Color: " + String(rgbColor.r) + " " + String(rgbColor.g) + " " + String(rgbColor.b));
    }
    leds[i].r = rgbColor.r;
    leds[i].g = rgbColor.g;
    leds[i].b = rgbColor.b;
  ...
  delayMilliseconds(50);
  if(!isTaskRunning)
    allClients[0].send("{\"status\":{\"command\":\"set-music\",\"data\":\"music-set\"}}");
}
...
void setNeoPixelBrightness(int iLed, int brightnessValue)
{
  ...
  rgbToHsv(rgbColor, hsvColor);
  hsvColor.v = static_cast<double>(brightnessValue / 255.0);
  hsvToRgb(hsvColor, rgbColor);
  leds[iLed].setRGB(rgbColor.r, rgbColor.g, rgbColor.b);
  FastLED.show();
  delayMilliseconds(50);
  if(!isTaskRunning)
...
void getNeoPixelBrightness(int iLed) {
  ...
  rgbToHsv(rgbColor, hsvColor);
  if (!isTaskRunning)
    allClients[0].send("{\"data\":" + String(static_cast<int>(hsvColor.v * 255)) + "}");
}
